...
WORKDIR /src
COPY ["MyFi.TheBadlands.csproj", "MyFi.TheBadlands/"]
RUN dotnet restore "MyFi.TheBadlands/MyFi.TheBadlands.csproj"

WORKDIR "/src/MyFi.TheBadlands"
COPY . .
RUN dotnet build "MyFi.TheBadlands.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "MyFi.TheBadlands.csproj" \
...
Execute unit tests
```
    cd "MyFi.TheBadlands/test/MyFi.TheBadlands.Tests"
    dotnet test
```

Deploy application
```
    cd "MyFi.TheBadlands/src/MyFi.TheBadlands"
